...
-- 1) Snapshot table for final results
...
-- Owner can read results of their game (join via game_sessions)
...
-- 2) Lifetime totals on profiles (whole dollars)
...
-- 4) Robust, idempotent finalize RPC (accepts *_dollars or *_cents). Upserts snapshot + updates totals.
...
begin
  ...
  for r in select * from jsonb_array_elements(p_results) loop
    ...
    -- Lifetime totals (sessions always +1; wins only for winners)
...
revoke all on function public.profile_finalize_game(uuid, uuid, jsonb) from public;
grant execute on function public.profile_finalize_game(uuid, uuid, jsonb) to authenticated;

-- 5) Stable source for Profile page: derive P/L
