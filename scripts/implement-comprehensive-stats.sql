...
-- 1) Snapshot table: one row per (game, profile) with final numbers in dollars
...
-- Owner of the game can read its results
...
-- 2) Ensure lifetime totals exist on profiles (whole dollars)
...
-- 3) Idempotency ledger (preserve existing data)
...
-- 4) Robust, idempotent finalize RPC (accepts *_dollars OR *_cents)
...
begin
  ...
  for r in select * from jsonb_array_elements(p_results) loop
    ...
    -- snapshot for Closed Summary (idempotent upsert)
...
-- 5) Stable source for Profile page: DERIVE P/L in a view
