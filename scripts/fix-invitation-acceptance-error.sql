-- Fix the COALESCE type conversion error in accept_game_invitation function

-- Drop the existing function first
DROP FUNCTION IF EXISTS public.accept_game_invitation(UUID);

-- Recreate the function with proper UUID array handling
...
BEGIN
  ...
  -- Handle invited_users array properly - keep as UUID array
  current_invited_users := COALESCE(game_record.invited_users, ARRAY[]::UUID[]);
  current_invited_users := current_invited_users || ARRAY[invitation_record.invitee_id];
  
  -- Update the game session
  UPDATE game_sessions 
  SET 
    players_data = updated_players,
    invited_users = current_invited_users,
    updated_at = now()
  WHERE id = invitation_record.game_session_id;
