-- COMPREHENSIVE DATABASE SCHEMA ANALYSIS AND FIXES (Fixed Type Mismatches)
-- ========================================================================
...
-- Fixed UUID/text comparison errors by ensuring proper column references
-- 6. CREATE comprehensive RLS policies with proper type handling
-- PROFILES policies - Fixed to only compare UUID columns
...
-- Fixed game_sessions policies to handle invited_users array with proper UUID casting
-- GAME_SESSIONS policies
CREATE POLICY "game_sessions_select_own" ON game_sessions 
FOR SELECT USING (
    auth.uid() = user_id OR 
    (invited_users IS NOT NULL AND auth.uid() = ANY(invited_users::uuid[])) OR
    EXISTS (
...
-- Improved auto-profile creation trigger with better error handling and type safety
...
-- Fixed profile creation query to handle proper data types
