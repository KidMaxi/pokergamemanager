...
SELECT
    ...
    pg_size_pretty(pg_total_relation_size(table_name) - pg_indexes_size(table_name)) AS table_data_size
FROM (
    SELECT table_name
...
-- Column Usage Analysis (Basic - requires more sophisticated tracking for definitive results)
...
AND pg_attribute.attnum > 0  -- Exclude system columns
AND NOT EXISTS (
    SELECT 1
    FROM pg_stats
    WHERE pg_stats.tablename = pg_class.relname
...
-- Check for Unused Indexes (requires pg_stat_all_indexes extension)
...
WHERE pg_stat_all_indexes.schemaname = 'public' AND pg_stat_all_indexes.idx_scan = 0
ORDER BY pg_stat_all_indexes.relname, pg_stat_all_indexes.indexrelname;

-- Check for Table Bloat (Estimate - requires more sophisticated analysis for definitive results)
SELECT
    schemaname,
    tblname,
    bs*ceil(reltuples/(bs/est_rec_size)) AS est_tblsize,
    pg_size_pretty(bs*ceil(reltuples/(bs/est_rec_size))) AS est_tblsize_pretty
FROM (
    SELECT
        schemaname,
        relname AS tblname,
        reltuples,
        bs,
        (4+tupw+pad)::numeric AS est_rec_size
    FROM (
        SELECT
            schemaname,
            relname,
            ...
            pg_block_size() AS bs,
            CASE
                WHEN avg_width > 0 THEN avg_width
                ELSE 6
            END AS avg_width,
            CASE
            ...
            END*8 AS pad,
            CASE
                WHEN attlen = -1 THEN 4
                ELSE attlen
            END AS attlen,
            CASE
                WHEN typalign = 'a' THEN 8
                ELSE 4
            END AS tupw
        FROM pg_stat_all_tables AS st
        JOIN pg_attribute AS a ON st.relid = a.attrelid
...
ORDER BY est_tblsize DESC;

-- Check for Duplicate Indexes (Name and Columns)
SELECT
    tablename,
    indexname,
...
FROM (
    ...
    ORDER BY tablename, indexname, attname
) AS subquery
...
AND NOT EXISTS (
    ...
    AND pg_index.indkey[1] = confkey[1]
);

-- Check for Large Tables without Indexes
SELECT relname, pg_size_pretty(pg_total_relation_size(relid)) AS size
FROM pg_catalog.pg_statio_all_tables
WHERE relname NOT IN (SELECT tablename FROM pg_catalog.pg_indexes WHERE schemaname = 'public')
AND pg_total_relation_size(relid) > 5 * 1024 * 1024
